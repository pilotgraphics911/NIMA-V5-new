...
async function attpCommand(sock, chatId, message) {
    ...
    if (!text) {
        await sock.sendMessage(chatId, { text: 'කරුණාකර .attp විධානයෙන් පසු පෙළ සපයන්න..' });
        return;
    }
    ...
    try {
        ...
        await sock.sendMessage(chatId, { text: 'ස්ටිකරය ජනනය කිරීමට අසමත් විය. කරුණාකර පසුව නැවත උත්සාහ කරන්න..' });
    }
}
