		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `\`\`\`@${sender.split('@')[0]} මෙහි සබැඳි අවසර නැත.\`\`\``,
					mentions: [sender] 
				});
				break;
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `\`\`\`@${sender.split('@')[0]} සබැඳි යැවීම නිසා ඉවත් කර ඇත.\`\`\``,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} අනතුරු ඇඟවීම් ${WARN_COUNT} කට පසු ඉවත් කරන ලදි\`\`\``,
						mentions: [sender]
					});
				} else {
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} අවවාදයයි ${warningCount}/${WARN_COUNT} සබැඳි පල කිරීමට\`\`\``,
						mentions: [sender]
					});
				}
				break;
