...
function loadAntibadwordConfig(groupId) {
    try {
        ...
        console.error('❌ අසාර්ථකයි. config loading වීම අසාර්ථකයි:', error.message);
        return {};
    }
}
...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `*ප්‍රති වචන සැලසුම*\n\n*.antibadword on*\nප්‍රති වචන සැලසුම ක්‍රියාත්මකයි\n\n*.antibadword set <action>*\nSet action: delete/kick/warn\n\n*.antibadword off*\nප්‍රති වචන සැලසුම අක්‍රියයි`
        });
    }
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '*ප්‍රති වචන සැලසුම දැනටමත් සක්‍රියයි*' });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return sock.sendMessage(chatId, { text: '*ප්‍රති වචන සැලසුම දැන් සක්‍රියයි.භාවිතා කරන්න .antibadword set <action> to customize action*' });
    }

    if (match === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return sock.sendMessage(chatId, { text: '*ප්‍රති වචන සැලසුම දැනටමත් අක්‍රියයි*' });
        }
        await removeAntiBadword(chatId);
        return sock.sendMessage(chatId, { text: '*මෙම සමූහයේ ප්‍රති වචන සැලසුම අක්‍රිය කරන ලදි*' });
    }

    if (match.startsWith('set')) {
        const action = match.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '*වැරදි ක්‍රියාවකි. තෝරන්න: delete, kick, or warn*' });
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `*ප්‍රති වචන සැලසුම සකස් කර ඇත: ${action}*` });
    }

    return sock.sendMessage(chatId, { text: '*වලංගු නොවන විධානයකි. භාවිතය බැලීමට .antibadword භාවිතා කරන්න.*' });
}
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    ...
    if (!antiBadwordConfig?.enabled) {
        console.log('මෙම සමූහයේ ප්‍රති වචන සැලසුම අක්‍රීයයි');
        return;
    }
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    try {
        ...
        console.error('පණිවිඩය මැකීමේ දෝෂයකි:', err);
        return;
    }
    ...
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `*@${senderId.split('@')[0]} නරක වචන වලට මෙහි අවසර නැත.*`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} නරක වචන භාවිතා කිරීම නිසා පයින් ගසා එලවා දමන ලදි.*`,
                    mentions: [senderId]
                });
            } catch (error) {
        ...
        case 'warn':
            ...
            if (warningCount >= 3) {
                try {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `*@${senderId.split('@')[0]} අනතුරු ඇඟවීම් 3 කට පසුව පයින් ගසා ඉවත් කරන ලදි.*`,
                        mentions: [senderId]
                    });
                } catch (error) {
                    console.error('අනතුරු ඇඟවීම් වලින් පසු පරිශීලකයා ඉවත් කිරීමේ දෝෂයකි:', error);
                }
            } else {
                await sock.sendMessage(chatId, {
                    text: `*@${senderId.split('@')[0]} අවවාදයයි ${warningCount}/3 ක් ඔබ වැරදි වචන බාවිතා කර ඇත*`,
                    mentions: [senderId]
                });
            }
            break;
